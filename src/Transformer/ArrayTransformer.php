...
class ArrayTransformer extends AbstractTransformer
{
    ...
    public function transform(FormInterface $form, array $extensions = [], $widget = null): array
    {
        ...
        if (empty($children)) {
            ...
            if (!$entryType) {
                throw new TransformerException(sprintf('Liform cannot infer the json-schema representation of a an empty Collection or array-like type without the option "allow_add" (to check the proptotype). Evaluating "%s',
                    $form->getName()));
            }
